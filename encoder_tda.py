...
class Encoder_tda(nn.Module):
    def __init__(self, img_size, patch_size, in_chans,
        ...
        self.head_list_recon = nn.Linear(28, embed_dims[-1])
        # self.sign = LBSign.apply
        self.norm = norm_layer(embed_dims[-1])  # + tda_dim
        self.head_list = nn.Linear(embed_dims[-1] * 2, embed_dims[-1] * 16)   # + tda_dim
        self.head_list1 = nn.Linear(embed_dims[-1] * 16, 1)
    ...
    def forward(self, x, x_tda):
        ...
        x_tda_recon = self.head_list_recon(x_tda)

        x = torch.cat([x, x_tda_recon], 2)

        x = F.relu(self.head_list(x))

        ACK = self.sigmoid(self.head_list1(x))
        ...
        # x = x.detach().cpu()
        # x_max = torch.max(x)
        # x_tmp = torch.div(x, x_max)

        # # quantize
        # x_tmp = torch.mul(x_tmp, 256)
        # x_tmp = x_tmp.clone().type(torch.int)
        # x_tmp = x_tmp.clone().type(torch.float32)
        # x_tmp = torch.div(x_tmp, 256)
        #
        # x = torch.mul(x_tmp, x_max)
...
def create_encoder_tda(**kwargs):
    model = Encoder_tda(**kwargs)
    return model
