...
class Decoder(nn.Module):
    def __init__(self, img_size, embed_dims, depths, num_heads, C,
        ...
        # self.dQ = nn.Linear(30,C)
        self.head_list = nn.Linear(2*C, embed_dims[0] + 28)
        self.head_list1 = nn.Linear(embed_dims[0] + 28, embed_dims[0])
    ...
    def forward(self, x, snr):
        ...
        x_tda = x[:, :, x.shape[2] - 28: x.shape[2]]
        # x_tda dim2 mean   (!!! negative value = -1)
        # x_tda = torch.mean(x_tda, dim=1)
