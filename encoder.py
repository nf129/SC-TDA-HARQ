...
class Encoder(nn.Module):
    def __init__(self, img_size, patch_size, in_chans,
        ...
        self.norm = norm_layer(embed_dims[-1] + 28)  # + tda_dim
        self.head_list = nn.Linear(embed_dims[-1] + 28, C)   # + tda_dim
        self.apply(self._init_weights)

    def forward(self, x, x_tda, snr):
        B, C, H, W = x.size()
        device = x.get_device()
        ...
        x_tda = torch.unsqueeze(x_tda, dim=1)
        x_tda = x_tda.repeat(1, x.shape[1], 1)
        x = torch.cat([x, x_tda], 2).float()
